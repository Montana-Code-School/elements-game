  ...
  clickHandler = (e) => {
    if (this.state.turn !== this.state.playerName && this.state.afterFlip === "") {
      ...
      if (((parent === "playerHand" || parent === "shadowActionModal") && this.state.player.hand[onClickedCardName] > 0) || (parent === "fireActionModal" && this.state.opponent.field[onClickedCardName] > 0) || (parent === "lightActionModal" && this.state.player.discard[onClickedCardName] > 0)) {
        this.closeModal();
        this.state.client.clickCard(
          e.currentTarget.className.split(" ")[2],
          this.state.room,
          this.state.afterFlip
        );
      ...
      this.setState({
        ...
        if (this.state.afterFlip === "shadowAction") {
          if (this.state.opponent.hand === 0) {
            this.modalContent(
              "closeButton",
              "There are no elements in your hand to discard.",
              this.state.client.clickCard.bind(this, "none", this.state.room, this.state.afterFlip)
            )
          } else {
      ...
      this.setState({
        ...
        if (this.state.afterFlip === "lightAction") {
          if (this.getCount(this.state.player.discard) === 0) {
            this.modalContent(
              "closeButton",
              "There are no elements in your discard.",
              this.state.client.clickCard.bind(this, "none", this.state.room, this.state.afterFlip)
            )
          } else {
            this.modalContent("noButton")
          ...
          if (this.getCount(this.state.opponent.field) === 0) {
            this.modalContent(
              "closeButton",
              "There are no elements on the opponents field",
              this.state.client.clickCard.bind(this, "none", this.state.room, this.state.afterFlip)
            )
          } else {
            this.modalContent("noButton")
    ...
    return (
      <Card className={classes.page}>
        <CustomModal
          ...
          closeModal={this.closeModal}></CustomModal>
